...
# an offset of 3, you would move three instructions forward, but change it
...
# --- Solution ---
#
# We start by reading the input file into a list of integers. We save a length
# of the list, as well as helper variables: steps counter and current index.
# Then in an infinte loop, as long as the index is in valid range, we read
# the offset (jump value from current index), increment the current field,
# then move the index and increment the steps counter. Finally, we return
