...
# many times forward around the track (that is, moving clockwise on spaces
# in order of increasing value, wrapping back around to 1 after 10).
...
#
# Solution:
# We read the input file and save only the last element in each line, which is
# the initial position of each player. Then we simply perform a loop as long
# as neither of players reached the goal score (1000) – in each step we choose
# a player who rolls, we calculate the new position and update the player's
# score. As final answer we return the smallest of scores times the number
# of rolls, which is three times the number of turns completed.
#
...
def main():
    ...
    while scores[PLAYER_1] < GOAL and scores[PLAYER_2] < GOAL:
        player = PLAYER_1 if turns % 2 == 0 else PLAYER_2
        roll = 3 * turns + 1 + 3 * turns + 2 + 3 * turns + 3
