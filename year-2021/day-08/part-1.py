...
# connected to segments randomly. Worse, the wire/segment connections
...
# example means that to render a 7, signal lines `d`, `a`, and `b` are on.
...
# of signal wires corresponds to each of the ten digits. Then, you can decode
# the four digit output value. Unfortunately, in the above example, all of the
# digits in the output value (`cdfeb` `fcadb` `cdfeb` `cdbaf`) use five
...
#
# Solution:
# While the description is overwhelming here, the actual solution for first
# part is very simple – we just need to read the input file, splitting every
# line on pipe character (|) and count how many words of length 2, 3, 4 and 7
