...
# you discover the phone number of a deep-sea marine biologist on a handwritten
...
# sea cucumber has moved (as it wasn't out of the way in time for the east-
# facing cucumber on the left to move), but both sea cucumbers on the right
...
#
# Solution:
# We read the input file as matrix of characters in Python (list of lists
# containing characters [strings of length 1). Then we perform the moving
# of matrix elements in a loop, repeating as long as there was any change
# done during current iteration. We need to consider moving of the right
# facing elements '>' first, then the elements facing down 'v'. The actual
# moving has to be done as separate action so we do not move the same element
# multiple times in single step. As final answer we print the number of
