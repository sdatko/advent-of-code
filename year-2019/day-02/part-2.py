...
# --- Solution ---
#
# The difference here is that we need to figure out input values for program
# to be set at position 1 and position 2. The key here it to catch that these
# values are addresses of computer's memory cells, which greatly reduces
# the possibilities – both must be the indices of the input list elements.
# Hence we run a search on all possible combinations of the indices.
# Finally we return an answer once the target output value was found.
#
...
def execute(program, noun, verb):
    ...
    while True:
        ...
        if opcode == 1:
            position1 = program[index + 1]
            position2 = program[index + 2]
            position3 = program[index + 3]

            value1 = program[position1]
            value2 = program[position2]
            program[position3] = value1 + value2
...
def main():
    ...
    max_address = len(program)

    for noun in range(max_address):
        for verb in range(max_address):
            output = execute(program.copy(), noun, verb)
