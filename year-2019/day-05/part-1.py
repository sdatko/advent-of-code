...
# a diagnostic program (your puzzle input). The TEST diagnostic program will
...
# in an instruction. Parameter modes are single digits, one per parameter, read
...
# the result of the test was from the expected value, where 0 means
# the test was successful. Non-zero outputs mean that a function is not working
...
# --- Solution ---
#
# We start by reusing the program from day 2 and extending it to support two
# additional opcodes – 3 for reading the inputs and 4 for writing the outputs.
# Then for existing opcodes 1 (addition) and 2 (multiplication) we introduce
# support for position and immediate modes: in first case, the argument we read
# is an address (index in code) where the value shall by taken from, while
# in the second case the argument is an explicitly given value. Finally,
# as an answer, we return the last output of our interpreted program.
#
