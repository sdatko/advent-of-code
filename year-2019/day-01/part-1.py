...
# --- Solution ---
#
# We start by reading the input file as list of number (spitting over newlines
# and casting each line to the integer). Then all we need is to calculate
# the sum of expression values: ( x // 3 ) - 2; for each entry number.
# As an answer we return the calculated sum.
#
...
def main():
    with open(INPUT_FILE, 'r') as file:
        masses = [int(mass)
                  for mass in file.read().strip().split('\n')]
