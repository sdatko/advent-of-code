...
# no sign of the Chief Historian, the engineers there run up to you as soon
# as they see you. Apparently, they still talk about the time Rudolph was
...
# You turn to check if The Historians are waiting for you, but they seem
# to have already divided into groups that are currently searching every
...
# --- Solution ---
#
# We start by reading the input file into a list of reports (list of integers).
# Then we process each report – for each one, we calculate the differences
# between levels and we verify if all the differences are of the same sign
# and not bigger than 3 (so either in [+1, +2, +3] or [-1, -2, -3] ranges);
# when the condition is meet, we count the report as safe.
# Finally, as an answer we print the number of safe reports.
#
