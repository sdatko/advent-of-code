...
# to Christmas, and while The Historians continue their search of this
...
# the rules. Like the first update, they also do not include every page number,
...
# --- Solution ---
#
# We start by reading the input file into two collections – rules and updates,
# by splitting over two newlines and then by the pipe and comma characters,
# respectively (with additional mapping to integer values). Then we process
# all numbers from a collection of updates, for each numbers verifying whether
# they satisfy the defined rules. For each rule, we check if it is applicable
# (both values in the rule are within the number) and if so – if the order
# of values is correct (i.e. the number to be before appears earlier than
# than the other number). If no rule is broken, the numbers are in right order
# and we pick the middle number to build the answer. Finally, we return the sum
# of all middle numbers we found.
#
...
def main():
    ...
    for numbers in updates:
        for rule in rules:
            ...
            if not before < after:  # wrong order – stop current processing
                break
