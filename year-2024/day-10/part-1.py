...
# --- Day 10: Hoof It ---
#
# You all arrive at a Lava Production Facility on a floating island
# in the sky. As the others begin to search the massive industrial
# complex, you feel a small nose boop your leg and look down to discover
...
#
# The topographic map indicates the height at each position using
# a scale from 0 (lowest) to 9 (highest). For example:
#
#   0123
#   1234
#   8765
...
# these positions will always have height 0. Assembling more fragments
...
# --- Solution ---
#
# We start by reading the input file into a 2-dimensional grid of levels,
# by splitting over newlines and mapping all characters into integers, using
# a custom mapping that returns -1 for non-digit characters (this way we can
# easily try example maps that contain dots). Next, we browse the grid to find
# all possible locations of the trail heads – we store the locations in a list
# of paths. Then, we process the list of paths like a queue, for each path
# attempting to find the next possible location, until we reach the trail goal
# (location at level 9). For each goal reached, we save the information about
# the starting location that led us to the goal. Finally, as an answer we print
# the number of all unique start->goal variants that are possible.
#
...
def main():
    with open(INPUT_FILE, 'r') as file:
        grid = tuple(tuple(map(dot_int, line))
    ...
    paths = []
    trails = set()  # unique (start, goal) variants

    for y in range(0, max_y):
        for x in range(0, max_x):
            if grid[y][x] == TRAILHEAD:
                paths.append([(x, y)])
