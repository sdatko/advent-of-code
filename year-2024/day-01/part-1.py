...
# locations that are historically significant to the North Pole; a group
# of Senior Historians has asked you to accompany them as they check
...
# --- Solution ---
#
# We start by reading the input file into a collection (tuple) of integers.
# Then we split that collection into two tuples, by picking every second item.
# Both newly created collections are then also sorted. Next, we process pairs
# of numbers from both collections, each time subtracting the smaller number
# in pair from the bigger number in a given pair. Finally, as an answer,
# we print the sum of calculated differences in numbers.
#
...
def main():
    ...
    for pair in zip(list1, list2):
        distances.append(max(pair) - min(pair))
