...
# or directional buttons to control the claw, these machines have two buttons
...
#   initial position, it would need to move exactly 8400 units along
#   the X axis and exactly 5400 units along the Y axis to be perfectly
#   aligned with the prize in this machine.
#
...
# --- Solution ---
#
# We start by reading the input file into a list of machines, each defined
# as a tuple of 6 numbers (Ax, Ay, Bx, By, Cx, Cy) – by filtering the data
# to remove all unnecessary elements and keeping only the numeric values.
# Then we iterate through all defined machines and attempt to find a solution
# using naive search within the given boundaries – that each button is pressed
# up to given upper number of times. When at some point the reached location
# matches the prize location, we calculate the cost of tokens to reach it.
# Under the given conditions (all coefficients are positive and the numbers
# of button presses are integers [obviously!]), there is exactly one possible
# solution at most (there are many paths that lead to the prize, but each
# will contains the same number of A and B buttons presses, just in different
# order, e.g. AAAB, AABA, ABAA, BAAA). Finally, as an answer we return
# the total number of tokens costs for all machines with reachable prizes.
#
...
def main():
    ...
    for machine in machines:
        ...
        for times_A in range(TIMES + 1):
            for times_B in range(TIMES + 1):
                location = btn_A * times_A + btn_B * times_B

                if location == prize:
                    cost = times_A * COST_A + times_B * COST_B

        if cost:  # solution exists
            tokens += cost
