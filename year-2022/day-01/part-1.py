...
# --- Solution ---
#
# First step is to read the input file with splitting it by empty line (\n\n).
# This way we obtain list of raw entries for each elf – so then we split each
# such entry by newline (\n) to get list of values for each elf. The last part
# is mapping obtained lists of values to integers, so we have real numbers
# and can perform mathematical operations on those.
# Then, all that remains, is to calculate sum of values for each elf and select
# the biggest element from all calculated values.
#
...
def main():
    ...
    most = max([sum(elf) for elf in elves])
