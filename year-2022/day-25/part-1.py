...
# the equipment the balloons brought: many hot air balloon kits, some fuel
# tanks, and a fuel heating machine.
#
...
# rate. This amount is simply the sum of the fuel requirements of all
# of the hot air balloons, and those fuel requirements are even listed
...
# on the shoulder. Apparently, the fuel requirements use numbers written
# in a format the Elves don't recognize; predictably, they'd like your help
# deciphering them.
#
...
# have a ones place, a tens place, a hundreds place, and so on, where the digit
...
# a snowflake-shaped container into one of the fuel tanks, thank
# the support representative for their time, and disconnect the call.
#
...
# --- Solution ---
#
# We start by reading the input into a list of strings, by simply splitting
# the file over newlines. Then for every string being a SNAFU number we run
# a function that decodes into a decimal integer and add it to the total sum.
# Next we run the reversed function, that converts the decimal integer
# into a SNAFU string, which is our answer.
# The tricky thing in conversion was to realise that we simply need to shift
# every remainder by value 2, so when calculating remainders (4, 3, 2, 1, 0)
# we get what we need (2, 1, 0, -1, -2).
#
...
def SNAFUtoDEC(string):
    value = 0

    for i, symbol in enumerate(reversed(string)):
        value += DECODE[symbol] * (5 ** i)

    return value


def DECtoSNAFU(integer):
    value = ''
    ...
    while True:
        ...
        remainder = (integer + 2) % 5 - 2
        integer = (integer + 2) // 5

        value = ENCODE[remainder] + value
...
def main():
    with open(INPUT_FILE, 'r') as file:
        numbers = [number for number in file.read().strip().split('\n')]

    total = 0

    for number in numbers:
        total += SNAFUtoDEC(number)
