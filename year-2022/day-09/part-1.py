...
# --- Day 9: Rope Bridge ---
#
# This rope bridge creaks as you walk along it. You aren't sure
# how old it is, or whether it can even support your weight.
#
...
# the tail, the tail must also move one step in that direction so it remains
# close enough:
#
#   .....    .....    .....
#   .TH.. -> .T.H. -> ..TH.
...
# a series of motions. Assume the head and the tail both start
...
# --- Solution ---
#
# We read the input file as list of motions to perform, each defined as tuple
# of direction indicator and the distance, splitting the data by newlines.
# We prepare a dictionary with vectors of movements for defined directions.
# Then we proces the motions in a loop. For each entry, we move the head
# to a given direction and we check whether the new head position is distant
# for more than 1 unit on any axis – if so, we move also the tail to previous
# position of the head. We also add the updated position of the tail to a set
# of visited locations. Finally, as an answer, we return the count of elements
# in the set.
#
...
def main():
    ...
    for direction, distance in motions:
        ...
        for step in range(distance):
            ...
            if any([abs(head_position[0] - tail_position[0]) > 1,
                    abs(head_position[1] - tail_position[1]) > 1]):
                tail_position[0] = head_position[0] - move[0]
                tail_position[1] = head_position[1] - move[1]
