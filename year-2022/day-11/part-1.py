...
# will throw your items. After some careful observation, you realize
...
# and 105 times. The level of monkey business in this situation can be
# found by multiplying these together: 10605.
#
# Figure out which monkeys to chase by counting how many items they inspect
# over 20 rounds. What is the level of monkey business after 20 rounds
# of stuff-slinging simian shenanigans?
#
#
# --- Solution ---
#
# We start by reading the input to a list of definitions dictionaries.
# Each definition is separated with an empty line, so we split the file
# by '\n\n' to get such list. Additionally we perform some replaces to align
# the data into easier to parse form. Each definition then we split by colon
# to obtain the dictionary. TL;DR The transformation is as follows:
#   Monkey 0:                                 ID: monkey 0:
#     Starting items: 79, 98                  Starting items: 79, 98
#     Operation: new = old * 19       --->    Operation: new = old * 19
#     Test: divisible by 23                   Test: divisible by 23
#       If true: throw to monkey 2            If true: throw to monkey 2
#       If false: throw to monkey 3           If false: throw to monkey 3
# Then we process all definitions to produce the structure for each monkey:
# – last 8 characters under 'If true' and 'If false' indicates the target
#   monkey ID where the calculated worry level will be appended,
# – the last value under 'Test' specifies the divisor we use for calculated
#   worry level (after casting it to an integer),
# – the 'Starting items' contains an initial list of worry level values
#   (after splitting and mapping the values to integers),
# – the 'Operation' describes how to process each worry level for a new value
#   (with Python magic, I construct a lambda and eval it to get a function).
# Having a structure for each monkey prepared, we perform a loop – for a given
# number of rounds, we iterate over defined monkeys: taking every defined item
# and calculating new worry level value and appending it to new target monkey
# depending if the value is divisible or not by a given number. We count every
# time each monkey processes a value.
# Finally, after all rounds, we take and multiply the two biggest numbers
# that indicate how many values each monkey processed. This is our answer.
#
