...
#
# Situations like this must be why the Elves prioritized getting
# the communication system on your handheld device working. You pull it out
...
# --- Solution ---
#
# We start by reading the input file as a list of instructions (every line
# in file is a single instruction). Then we process the instructions:
# for each one we increase the cycle number and we attempt to measure
# the signal strength (provided that conditions are met).
# Depending on the instruction, this happens once (for noop) or twice (addx).
# For the addx instruction we additionally change the value of register X.
# Finally, as an answer, we return the sum of signals that meet the condition.
#
...
def main():
    ...
    def signal_strength(cycle, register_X):
        if ((cycle - 20) % 40) == 0:
