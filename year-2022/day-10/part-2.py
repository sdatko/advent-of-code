#!/usr/bin/env python3
#
# --- Day 10: Cathode-Ray Tube / Part Two ---
#
...
# the horizontal position of the middle of that sprite. (In this system,
# there is no such thing as "vertical position": if the sprite's horizontal
...
# --- Solution ---
#
# The difference in this part is that of measuring the signal strength,
# we implement the drawing operation. Also the order of operations is swapped:
# first there is attempt to draw, then the cycle is increased (which simulates
# that the drawing took some time). The drawing itself occurs only if
# the current processed pixel (calculated with cycle number) is within
# the active sprite (determined by the register X value).
# Finally as an answer we assemble the screen by split the screen string
...
def main():
    ...
    cycle = 0
    register_X = 1
    screen = list(PIXEL_DARK * (WIDTH * HEIGHT))

    def draw(cycle, register_X):
        sprite = [(register_X - 1) % len(screen),
                  (register_X) % len(screen),
                  (register_X + 1) % len(screen)]

        if (cycle % WIDTH) in sprite:
            screen[cycle % len(screen)] = PIXEL_LIGHT
