...
# two compartments, so the first half of the characters represent items
...
# --- Solution ---
#
# We start with reading the input file as list of words, as each word
# represents the rucksack. Then we define a map of priorities for items,
# so it looks like a: 1, b: 2, ..., z: 26, A: 27, B: 28, ..., Z: 52.
# Then we process every word from the input list:
# – we generate two sets, from the first and from the second half of the word,
# – we find the common element by calculating the sets intersection,
# – we add the priority of the common element to the total sum.
# Finally we just print the sum of priorities added.
#
...
def main():
    ...
    priority = {chr(ord(letter) + i): start + i
                for letter, start in [('a', 1), ('A', 27)]
                for i in range(26)}
    ...
    for rucksack in rucksacks:
        size = len(rucksack) // 2
        first = set(rucksack[:size])
        second = set(rucksack[size:])

        common_item = set.intersection(first, second)
