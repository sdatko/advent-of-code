...
# near the ferry dock. "Boat races! Open to the public! Grand prize is
# an all-expenses-paid trip to Desert Island!" That must be where the sand
...
#   Then, the boat will travel at a speed of 1 millimeter per millisecond
#   for 6 milliseconds, reaching a total distance traveled of 6 millimeters.
# – Hold the button for 2 milliseconds, giving the boat a speed of
#   2 millimeters per millisecond. It will then get 5 milliseconds to move,
...
#   until you let go of the button. Please make sure you let go
...
# --- Solution ---
#
# We start by reading the input file into two lists of integers: total times
# from the first input line and the distances from the second line in file.
# Then we analyse each race (a pair of time and distance to beat), verifying
# for each possible waiting time, whether we will travel bigger distance
# than the given record. The velocity is a linear function of waiting time,
#   Velocity(waiting_time) = waiting_time,
# and the travel time for each velocity is what remains from the total time,
#   travel_time = total_time - waiting_time,
# then the total travelled distance (s = V * t) can be written here as
#   travelled = Velocity * travel_time
#   travelled = waiting_time * (total_time - waiting_time).
# Finally, as an answer we return the multiplied numbers of ways we can win
# each race.
#
...
def main():
    with open(INPUT_FILE, 'r') as file:
        ...
        distances = list(map(int, file.readline().strip().split()[1:]))
    ...
    for index, race in enumerate(races):
        total_time, distance = race

        for waiting_time in range(total_time):
            travelled = waiting_time * (total_time - waiting_time)
