...
# he would be: managing a giant "garden" that looks more to you like a farm.
#
...
# soon; we only turned off the water a few days... weeks... oh no."
...
# production problem. The latest Island Island Almanac just arrived
...
# to use with which seeds, which water to use with which fertilizer,
# and so on.
#
...
# of 98, and a range length of 2. This line means that the source range starts
...
#   99    51
#
# With this map, you can look up the soil number required
# for each initial seed number:
...
# --- Solution ---
#
# We start by reading the input file, fetching the first line into a list
# of integers (seeds) to process and the rest of the file into dictionary,
# where the key is the name of mapping to perform (e.g. seed-to-soil)
# and the values are lists of tuples specifying the given transformations
# (destination, source, length). Then in a loop we proces all defined seeds,
# for each one performing a series of transformations, using a helper function
# decode() that for a given value determines whether it lies within one of
# given mapping's ranges and calculating the appropriate return value.
# After a series of transformations for each seed, we save the lowest obtained
# value and return it as an answer.
#

INPUT_FILE = 'input.txt'


def decode(value, mappings):
    for destination, source, length in mappings:
        if source <= value < source + length:
            return destination + (value - source)

    else:  # value did not fall into any defined mappings – pass it through
        return value
...
def main():
    ...
    for seed in seeds:
        ...
        location = decode(location, maps['seed-to-soil'])
        location = decode(location, maps['soil-to-fertilizer'])
        location = decode(location, maps['fertilizer-to-water'])
        location = decode(location, maps['water-to-light'])
        location = decode(location, maps['light-to-temperature'])
        location = decode(location, maps['temperature-to-humidity'])
        location = decode(location, maps['humidity-to-location'])

        if not closest_location or closest_location > location:
