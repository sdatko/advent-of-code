...
# a prediction of the next value in each history. To do this, start by
...
# you just generated as the input sequence. Once all of the values in your
# latest sequence are zeroes, you can extrapolate what the next value
...
#         0   0   0
#
# Then, following the same process as before, work out the next value
# in each sequence from the bottom up:
#
#   1   3   6  10  15  21  28
#     2   3   4   5   6   7
#       1   1   1   1   1
...
# --- Solution ---
#
# We start by reading the input file into a list of sequences (the lists of
# integer values), by splitting the file content over newlines and then each
# line is split and mapped into integers. Next we process each input sequence,
# producing all possible sub-sequences (derivatives, e.g. for [a, b, c, d, e]
# we create [b-a, c-b, d-c, e-d], then [(c-b)-(b-a), (d-c)-(c-b), (e-d)-(d-c)],
# and so on...) until all the differences in sub-sequences are equal to zero.
# Then we find the predicted next value by going through all the produced
# sub-sequences in reversed order, every time taking the sum of last element
# of sub-sequence and the current predicted value; once processed all produced
# sub-sequences, the predicted next value is saved into a list of values.
# Finally, as an answer we return the sum of all saved predicted values.
#
...
def main():
    ...
    for sequence in sequences:
        ...
        while subsequences:
            ...
            predicted_value = (subsequence[-1] + predicted_value)
