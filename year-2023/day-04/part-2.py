...
# any more cards. (Cards will never make you copy a card past the end
# of the table.)
#
...
# --- Solution ---
#
# The difference here is that we also need to record the cards numbers.
# Then, first, we build a dictionary of scores, where we track the number
# of cards copies we will obtain (initially: 1 for each card given in input).
# Next we perform a similar loop like in part 1, however this time for each
# score (number of common cards) we increment the number of current cards
# using a following pattern: for current N cards of number X and score Y
# we add N to cards X+1, X+2, ..., X+Y. Finally, as an answer, we return
# the total number of all cards obtained.
#
...
def main():
    ...
    # initial scores for original cards
    for number in cards.keys():
        scores[number] = 1

    # additional scores
    for number, (winning, drawn) in cards.items():
        scored = len(set.intersection(winning, drawn))
