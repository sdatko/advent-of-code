...
# appears above you! The gondola carries you to the surface of
...
# with a better naming scheme. Tell you what: if you can help me with something
...
# a list of numbers you have. You organize the information into a table
# (your puzzle input).
#
...
# --- Solution ---
#
# We start by processing the input file into a list of two sets (the winning
# numbers and the drawn numbers). Then we iterate over the list of elements,
# each time calculating the number of common elements in both sets and
# increasing the number of total points if there was at least one common.
# Finally we simply return the total points scored.
#
...
def main():
    with open(INPUT_FILE, 'r') as file:
        cards = [list(map(lambda x: set(map(int, x.split())),
                          line.split(': ')[1].split(' | ')))
                 for line in file.read().strip().split('\n')]

    total_points = 0

    for winning, drawn in cards:
