...
# of the module is preceded by a symbol identifying its type, if any.
...
# which, because it has only one input, acts like an inverter (it sends
...
# now that both flip-flops are on and con remembers a high pulse from each
# of its two inputs, pushing the button a second time does something different:
#
#   button -low-> broadcaster
#   broadcaster -low-> a
#   a -low-> inv
...
# How many pulses got sent as a result (including the pulses sent
# by the button itself)?
#
# In the first example, the same thing happens every time the button
# is pushed: 8 low pulses and 4 high pulses are sent. So, after pushing
# the button 1000 times, 8000 low pulses and 4000 high pulses are sent.
...
# What do you get if you multiply the total number of low pulses sent
# by the total number of high pulses sent?
#
#
# --- Solution ---
#
# We start by reading the input file into a mapping of modules (input->outputs)
# by splitting the content over newlines and then each line on the `->` string.
# Moreover, from the modules we distinguish the flip-flops and conjuctions
# – storing them as map of their states (for flip-flops) and states of their
# watched inputs (for conjunctions). Then, in a loop, for each button press
# we declare that queue of states with initial signal and process the queue
# as long as there is any signal left. In each processing, we take the first
# signal from the list (in order that it was added to queue) and determine
# its next output. First, we analyze if there is any mapping/module given
# for the current state – if not, the signal there ends; otherwise, we are
# either in flip-flop or in a conjunction, where the altered signal level
# shall be calculated. Then, we take the output elements for the current state
# module and we create new states with new signal level values. Every time
# there is a new state added to queue, we also count for which state (low/high)
# it happened. Finally, we multiply the counts and return the result as answer.
#

INPUT_FILE = 'input.txt'

BUTTON_PRESSED_TIMES = 1000
...
def main():
    ...
    for _ in range(BUTTON_PRESSED_TIMES):
        # Button pressed
        states = [(START_MODULE, START_STATE, None)]
        outputs[START_STATE] += 1
